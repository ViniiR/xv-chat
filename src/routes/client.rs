...
pub async fn bundle() -> Option<NamedFile> {
    NamedFile::open("client/dist/main.bundle.js").await.ok()
}

#[get("/<file>", rank = 2)]
pub async fn file(file: PathBuf) -> Option<NamedFile> {
    let Some(ext) = file.extension() else {
        return NamedFile::open("client/dist/index.html").await.ok();
    };

    match ext.to_str().expect("no file extension") {
        "svg" | "jpeg" | "jpg" | "png" | "ico" | "gif" => {
            NamedFile::open(format!("client/dist/{}", file.to_str().expect("no file")))
                .await
                .ok()
        }
        _ => {
